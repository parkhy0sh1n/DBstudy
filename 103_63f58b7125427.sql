...
*/
-- 1) 사용자 함수 작성
CREATE OR REPLACE FUNCTION GET_STUDENT(ID STUDENT.USERID%TYPE)
RETURN VARCHAR2
IS
    STNUM STUDENT.STUDNO%TYPE;
BEGIN
    SELECT STUDNO
      INTO STNUM
      FROM STUDENT
    ...
    RETURN STNUM;
END;
-- 2) 아이디가 'jun123'인 학생의 학번을 조회하는 쿼리문을 작성
...
*/
-- 1) 프로시저 작성
CREATE OR REPLACE PROCEDURE MY_PROC(DEPT_NO IN DEPARTMENT.DEPTNO%TYPE)
IS
PROF_NO DEPARTMENT.PROFNO%TYPE;
BEGIN
    SELECT PROFNO
      INTO PROF_NO
      FROM DEPARTMENT
...
-- 2) 학과번호가 101인 학과를 대상으로 프로시저 실행 방법
EXECUTE MY_PROC(101);
